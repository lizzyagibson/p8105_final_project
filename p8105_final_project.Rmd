...
all_data <- left_join(school_data, lead_data, by = "school_number") %>% 
  ...
  mutate(lead = ifelse(lead_percent > median(lead_percent, na.rm = TRUE), "High", "Low"))
...
pal_lead <- colorFactor(c("#C77CFF", "#00BA38"), all_data$lead)
...
all_data %>%
  ...
  addCircles(~longitude, ~latitude, 
             label = ~content, color = ~pal_lead(lead)) %>% 
  addLegend(position = "bottomright", pal = pal_lead, values = ~lead,
