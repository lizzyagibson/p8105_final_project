...
score_data <- GET("https://data.cityofnewyork.us/resource/stka-4ti9.csv", query = list(`$limit` = 25000)) %>% 
  content("parsed") %>% 
  clean_names() %>% 
  filter(year == 2017 & grade == "All Grades") %>% 
...
all_data <- left_join(school_data, lead_data, by = "school_number") %>% 
  ...
  mutate(all_minority = ifelse(white > .5, "White", 
                               ifelse(black > .5, "Black",
                                      ifelse(asian > .5, "Asian",
                                             ifelse(hispanic > .5, "Hispanic",
                                                    ifelse(other_race > .5, "Other", "Minority"))))))
```
...
* all_minority = categorical, gives name of any race > .5
