...
### Map of All Schools in Dataset
  
```{r}
map_data %>% 
  leaflet() %>% 
  addTiles() %>% 
  addMarkers(~as.numeric(longitude), ~as.numeric(latitude), popup = ~as.character(school_number), label = ~as.character(school_number))
```

### Schools Without Lead Above Standard
...
### Schools With Lead Above Standard
